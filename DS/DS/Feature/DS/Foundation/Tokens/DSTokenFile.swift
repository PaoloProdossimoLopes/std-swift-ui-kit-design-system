...
extension DSTokenFile where Value: Decodable {
    func getFileDecodableValue<Values: Decodable>(
        forResource name: String? = nil,
        ofType ext: String? = nil
    ) -> Values? {
        guard let tokenDict = getFileValue(forResource: name, ofType: ext),
              let dictValue = tokenDict[rawValue] as? Values else {
            return nil
        }
        ...
        do {
            ...
            return try JSONDecoder.new.decode(Values.self, from: json)
        } catch {
